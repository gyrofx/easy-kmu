...
interface InvoiceState {
	...
	addItem: () => void;
	updateItem: (
		item: { pos: string; text: string; price: number },
		index: number,
	) => void;
	...
	persist<InvoiceState>(
		(set) => ({
			...
			removeProjectItem: (index: number) =>
				set(
					produce((state: InvoiceState) => {
						state.invoice.project = state.invoice.project.filter(
							(_, i) => i !== index,
						);
					}),
				),
			moveProjectItem: (fromIndex: number, toIndex: number) =>
				set(
					produce((state: InvoiceState) => {
						state.invoice.project = move(
							state.invoice.project,
							fromIndex,
							toIndex,
						);
					}),
			...
			addItem: () =>
				set(
					produce(
						(state: InvoiceState) =>
							void state.invoice.items.push({ pos: "", text: "", price: 0.0 }),
					),
				),
			updateItem: (
				item: { pos: string; text: string; price: number },
			...
			removeItem: (index: number) =>
				set(
					produce(
						(state: InvoiceState) => void state.invoice.items.splice(index, 1),
					),
				),
			...
			addSnippet: (snippet: Snippet) =>
				set(
					produce(
						(state: InvoiceState) => void state.invoice.snippets.push(snippet),
					),
				),
			...
			removeSnippet: (index: number) =>
				set(
					produce(
						(state: InvoiceState) =>
							void state.invoice.snippets.splice(index, 1),
					),
				),
			...
			moveSnippet: (fromIndex: number, toIndex: number) =>
				set(
					produce((state: InvoiceState) => {
						state.invoice.snippets = move(
							state.invoice.snippets,
							fromIndex,
							toIndex,
						);
					}),
								...
								<Box key={index} sx={{ display: "flex", flexDirection: "row" }}>
									<Box sx={{ display: "flex", gap: 2 }}>
										<TextField
											...
											variant="outlined"
											onChange={(ev) =>
												updateProjectItem(index, [ev.target.value, value])
											}
											value={key}
										/>
										...
										<TextField
											...
											variant="outlined"
											onChange={(ev) =>
												updateProjectItem(index, [key, ev.target.value])
											}
											value={value}
										/>
									</Box>
									...
									<Box sx={{ display: "flex", gap: 0 }}>
										...
										<Box>
											<IconButton
												onClick={() => moveProjectItem(index, index - 1)}
											>
												<KeyboardArrowUp />
											</IconButton>
										</Box>
					...
					<CardContent>
						...
						>
							...
							<TextField
								...
								onChange={(ev) =>
									updateItem(
										{ text: ev.target.value, pos: item.pos, price: item.price },
										index,
									)
								}
								multiline
					...
					<CardContent>
						...
						<Box sx={{ display: "flex", flexDirection: "row", gap: 2, mb: 2 }}>
							<TextField
								...
								variant="outlined"
								onChange={(ev) =>
									updateSnippet({ ...item, text: ev.target.value }, index)
								}
								multiline
								rows={10}
								value={item.text}
				...
				<CardContent>
					{invoice.snippets.length === 0 && (
						<>
							...
							<Typography gutterBottom variant="body1" component="div">
								Noch keine Textbausteine vorhanden. Füge eine Textbaustein
								hinzu.
							</Typography>
						</>
					)}
					...
					>
						{preparedSnippets.map((snippet, index) => (
							...
							>
								<ListItemText
									primary={snippet.label}
									secondary={snippet.text.slice(0, 50)}
								/>
							</MenuItem>
						))}
					</Menu>
