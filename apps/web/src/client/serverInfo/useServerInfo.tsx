...
export const ServerInfoContext = createContext(
	undefined as ServerInfo | undefined,
);

export function ServerInfoContextProvider({
	children,
		...
		return (
			<FatalError
				message={
					<>
						<p>
							An unexpected error has occurred. Please try reloading the app.
						</p>
	...
	if (serverInfo.value && serverInfo.value.status === 200) {
		return (
			<ServerInfoContext.Provider value={serverInfo.value.body}>
				{children}
...
export function useServerInfo() {
	...
	if (!serverInfo) {
		throw new Error(
			"server info not available. Make sure the ServerInfoProvider is used.",
		);
	}
