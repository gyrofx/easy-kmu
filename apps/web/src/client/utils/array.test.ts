...
import {
  ...
  removeLast,
  truthy,
} from '@/client/utils/array'

describe('truthy', () => {
  it.each([true, 1, 'x', {}, []])('%s is true', (value) => expect(truthy(value)).toBe(true))
  it.each([false, 0, '', undefined, null])('%s is false', (value) => expect(truthy(value)).toBe(false))
})
...
describe('length', () => {
  it.each(times(5))('array of length %s that length', (value) =>
    expect(isLength(times(value), value)).toBe(true),
  )
  it.each(times(5))('array of length %s not length + 1', (value) =>
    expect(isLength(times(value), value + 1)).toBe(false),
  )
})
...
describe('allOrNone', () => {
  ...
  it("throws an error if some match and some don't match the predicate", () => {
    expect(() => allOrNone(oneTwoThree, (n) => n <= 2, throwError)).toThrowError()
  })
})
