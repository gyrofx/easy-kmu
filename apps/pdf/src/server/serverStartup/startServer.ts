...
import { initStaticFiles } from "@/server/serverStartup/staticFiles";
...
import {
	...
	type ServerOptions as NodeServerOptions,
} from "node:https";
...
import { initApi } from "@/server/serverStartup/initApi";
...
export function createServer(app: Express, options: ServerOptions) {
	return options.https.enabled
		? createHttpsServer(httpsOptions(options), app)
