...
const defaultWebServerEnv = {
  WORKLOG_SESSION_SECRET: 'UAbzjdfPoiIKMKRiragYQR6USOBTntfouQEOm1kmZzUqpjOK7ZSSeKKlLa7Ho29JRfwe1UJhy2z',
  WORKLOG_AUTH0_DOMAIN: 'odimeter.example.com',
  WORKLOG_AUTH0_CLIENT_ID: '1234567890',
  WORKLOG_AUTH0_REDIRECT_URL: 'http://localhost:3000',
  WORKLOG_RUN_AS: 'webServer',
  WORKLOG_REDIS_URL: 'redis://localhost:6379',
...
describe('initOptsInternal', () => {
    ...
    describe('NODE_TLS_REJECT_UNAUTHORIZED', () => {
      describe('in production', () => {
        ...
        it('throws if value is an empty string', () => {
          expect(() => initOptsInternal({ ...env, NODE_TLS_REJECT_UNAUTHORIZED: '' })).toThrow()
        })

        it('throws if value if value is set', () => {
          expect(() => initOptsInternal({ ...env, NODE_TLS_REJECT_UNAUTHORIZED: '0' })).toThrow()
        })
      ...
      describe('in non production', () => {
        ...
        it('accepts empty value', () => {
          expect(() => initOptsInternal({ ...env, NODE_TLS_REJECT_UNAUTHORIZED: '' })).not.toThrow()
        })

        it('accepts string value', () => {
          expect(() => initOptsInternal({ ...env, NODE_TLS_REJECT_UNAUTHORIZED: '0' })).not.toThrow()
        })
      })
    })
  })
  ...
  describe('https opts', () => {
    ...
    it('throws an error with a missing ssl key', () => {
      expect(() =>
        initOptsInternal({ ...defaultWebServerEnv, HTTPS: 'true', SSL_CRT_FILE: 'file.crt' }),
      ).toThrow()
    })
  })

  it('parses the host', () => {
    expect(initOptsInternal({ ...defaultWebServerEnv, HOST: 'depart.localhost' }).host).toBe(
      'depart.localhost',
    )
  })

  it('parses the port', () => {
    expect(initOptsInternal({ ...defaultWebServerEnv, WORKLOG_PORT: '5555' }).port).toBe(5555)
  })
})
